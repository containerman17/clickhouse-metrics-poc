...
func NewChainSyncer(cfg Config) (*ChainSyncer, error) {
	...
	// Initialize index runner - one per chain
...
func (cs *ChainSyncer) writeBlocks(blocks []*rpc.NormalizedBlock) error {
	...
	// Update index runner with latest block (only once per batch)
	if len(blocks) > 0 {
		...
		if latestBlock != nil {
			...
			// IndexRunner is per-chain, so no race conditions
			blockTime := time.Unix(int64(timestamp), 0).UTC()
			if err := cs.indexRunner.OnBlock(uint64(latestBlockNum), blockTime); err != nil {
